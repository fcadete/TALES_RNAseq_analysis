...
genes_AD6 <- rownames(results_AD6[which(results_AD6$padj < 0.01), ])
genes_NLS3 <- rownames(results_NLS3[which(results_NLS3$padj < 0.01), ])
genes_SID4 <- rownames(results_SID4[which(results_SID4$padj < 0.01), ])

venn.plot <- draw.triple.venn(area1 = length(genes_AD6),
                              area2 = length(genes_NLS3),
                              area3 = length(genes_SID4),
                              n12 = length(intersect(genes_AD6,
                                                     genes_NLS3)),
                              n23 = length(intersect(genes_NLS3,
                                                     genes_SID4)),
                              n13 = length(intersect(genes_AD6,
                                                     genes_SID4)),
...
pdf(paste0("U2OS_results/", sample, "_top_BP_GO_results.pdf"), width = 12)
...
write.table(allRes.BP %>% filter(Fisher.classic < 0.01),
            file = paste0("U2OS_results/", sample, "top_BP_GO_results.txt"),
...
pdf(paste0("U2OS_results/", sample, "_top_MF_GO_results.pdf"), width = 12)
...
write.table(allRes.MF %>% filter(Fisher.classic < 0.01),
            file = paste0("U2OS_results/", sample, "top_MF_GO_results.txt"),
            sep = "\t", col.names = TRUE, row.names = FALSE, quote = FALSE)
