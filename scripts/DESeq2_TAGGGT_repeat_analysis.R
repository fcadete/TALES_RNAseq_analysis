...
pdf("HeLa_TAGGGT_repeat_analysis.pdf")
...
for (number_reps in 1:5) {
  ...
  gene_telrep_matches <- vmatchPattern(paste(rep("TAGGGT", number_reps), collapse = ""), geneSeqs)
...
for (number_reps in 1:5) {
  ...
  promoter_telrep_matches <- vmatchPattern(paste(rep("TAGGGT", number_reps), collapse = ""), promoterSeqs)
...
exon_matches <- t(sapply(exon_sequences, function(x) {
     ...
     sapply(1:5, function(e) {
              telrep_matches <- vmatchPattern(paste(rep("TAGGGT", e), collapse = ""), x)
